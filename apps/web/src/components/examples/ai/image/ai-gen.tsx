...
import { Card, CardContent } from "@microboat/web/components/ui/card";
...
import {
	...
	PopoverContent,
	PopoverTrigger,
} from "@microboat/web/components/ui/popover";
import {
	Select,
	SelectContent,
	SelectItem,
...
import { Slider } from "@microboat/web/components/ui/slider";
import { Switch } from "@microboat/web/components/ui/switch";
import { Tabs, TabsList, TabsTrigger } from "@microboat/web/components/ui/tabs";
...
function AIMultiModalGeneration() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const loadingTime =
				mode === "image" ? 3000 : mode === "video" ? 5000 : 7000;
	...
	const renderForm = () => (
		<form
			...
			className="flex flex-col gap-3 flex-1 p-4 justify-between"
		>
			<div className="space-y-3">
				...
				{advancedMode && (
					<div className="space-y-3 p-3 bg-zinc-50 dark:bg-zinc-800/30 rounded-xl">
						...
						<div className="space-y-1">
							<div className="flex items-center justify-between">
								<label htmlFor="steps" className="text-xs text-zinc-500">
									Steps
								</label>
								<span className="text-xs text-zinc-700 dark:text-zinc-300">
									{settings.steps || 30}
			...
			<div className="space-y-3">
				...
				<Button
					type="submit"
					className="w-full h-10 flex items-center justify-center gap-2 text-sm font-medium rounded-xl self-end"
				>
		...
		<div className="space-y-3 p-4 rounded-xl bg-zinc-50 dark:bg-zinc-800/50">
			...
			{mode !== "video" && (
				<div className="flex items-center justify-between gap-4">
					...
					<Select
						value={settings.pose}
						onValueChange={(value) => setSettings({ ...settings, pose: value })}
					>
						<SelectTrigger className="w-[160px] h-8 bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-700">
							<SelectValue />
						</SelectTrigger>
						...
						>
							...
							<div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-1.5">
								<div className="flex items-center gap-1">
									...
									{item.type === "avatar" && (
										<Cube className="w-3 h-3 text-white" />
									)}
				...
				{filteredItems.length === 0 ? (
					...
					filteredItems.map((item) => (
						<div
							...
							onKeyUp={(e) => {
								if (e.key === "Enter") {
									handleSelectHistoryItem(item.id);
								}
							}}
							onKeyDown={(e) => {
								if (e.key === "Enter") {
